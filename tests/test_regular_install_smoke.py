...
@unittest.skipUnless(
    os.getenv("YNN_RUN_REGULAR_INSTALL_SMOKE") == "1",
    "set YNN_RUN_REGULAR_INSTALL_SMOKE=1 to run regular install smoke verification",
)
class RegularInstallSmokeTests(unittest.TestCase):
    def test_ynn_prompt_runs_from_regular_install_outside_repo(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            run_cwd.mkdir()
            transcript_path.write_text("Regular install smoke transcript.\n", encoding="utf-8")
            ...
            installed_transcript_path = output_dir / "transcripts" / "abc123def45.txt"
            prompt_path = output_dir / "prompts" / "abc123def45_prompt.md"

            self.assertTrue(installed_transcript_path.exists())
